...
GEIH$tamaÒo_empresa <- factor(GEIH$tamaÒo_empresa)
GEIH <- GEIH %>%
...
#generaciÛn de grupo train - test
...
# especificaciÛn 3: salario ~ mujer+edad2+educacion_tiempo+tamaÒo_empresa

model3<-lm(log_salario_hora_imputado ~ edad + edad2 + educacion_tiempo + tamaÒo_empresa,data=train)
summary(model3)
test$model3<-predict(model3,newdata = test)
MSE_model3<-with(test,mean((log_salario_hora_imputado-model3)^2))
MSE_model3

# especificaciÛn 4: salario ~ mujer + edad + edad2 + educacion_tiempo + (interaccion entre edad y genero)
...
# especificacion 7: salario ~ mujer + edad + edad2 + edad3 + educacion_tiempo + educacion_tiempo2 + estrato + (InteracciÛn entre edad y genero)
...
stargazer(model1, model2, model3, model4, model5, model6, model7, 
          ...
          notes = c("Errores est·ndar en parÈntesis")
)
...
stargazer(ordenMSE, summary = FALSE, title= "Tabla 5.3 Modelos escogidos para validaciÛn cruzada", type = "text", out= "../views/ordenMSE.html")

#predecimos los errores para los modelos para realizar el gr·fico de predicciÛn de los errores
...
distribucion_error <- ggplot(test) +
  ...
  labs(title = "Distribuci√≥n de Errores de Predicci√≥n",
       x = "Error de Predicci√≥n", y = "Densidad",
       color = "Modelo") +
...
#Con estos test validamos la existencia de valores atÌpicos en las colas de 
#la distribuciÛn del modelo 6 y modelo 7
...
# De todos los modelos presentados, los que tienen un menor MSE son los modelos 6 y 7  Es decir, en donde se tiene un mejor performance en la predicciÛn. 
...
# Obtener los coeficientes y estadÌsticas del modelo
