...
GEIH<-GEIH %>% mutate(muj_res=lm(mujer~edad+edad2+educacion_tiempo+as.factor(ocupacion),GEIH)$residuals,#capturamos los residuales de mujer
                      sal_res=lm(log_salario_hora~edad+edad2+educacion_tiempo+as.factor(ocupacion),GEIH)$residuals) #capturamos los residuales del salario
...
stargazer(reg_w_fem, reg_fwl1, type="text",title="Tabla 4.1: Regresión Salario-Female", 
          ...
          add.lines=list(c("AIC", round(AIC(reg_w_fem),1), round(AIC(reg_w_fem2),1),'Variables de Control', 'No','Si')), notes="El modelo FWL (2) ha sido calculado con las variable de control edad, educación, y ocupación.", notes.align="l")
